...
//
// File Name:           ip_recip13p14v1_rtl.v
// Author:              Humphrey Lin
//
...
//                      the denominator ==2048 ~,     output reciprocal will be extened 10 extra bits  [o_prcis_idx[5] = 1]
// -FHDR -----------------------------------------------------------------------

module ip_recip13p14v1

#(
  parameter                         IPRCIS    = 13,             // input  precision
  parameter                         OPRCIS    = 14,             // output precision
...
localparam                          X1TC_BIT =  PRCIS_EXT == "LVL_0" ? X1SZ-OPRCIS+(IPRCIS-2) :
                                                PRCIS_EXT == "LVL_1" ? X1SZ-OPRCIS+(IPRCIS-4) :
                                                                       X1SZ-OPRCIS+1;// X1 truncate bit number
...
assign denm_norm = {LUT_ISZ{denm_lead_one[ 1]}} & {i_denm[ 1:0],4'b0} |
                   ...
                   {LUT_ISZ{denm_lead_one[12]}} & {i_denm[12:7]};
...
//--------------------------------------------------------------------------------
// A0 = Denm * X0   [0.5 < A0 < 2]
// u(.13) * u(1.7) -> adj to 1.13

assign  a0_prod     = i_denm * lut_recip;

assign  a0_prod_adj = {R0SZ{denm_lead_one[ 1]}} & {a0_prod[ 9: 0], 4'h0} |
                      {R0SZ{denm_lead_one[ 2]}} & {a0_prod[10: 0], 3'h0} |
                      {R0SZ{denm_lead_one[ 3]}} & {a0_prod[11: 0], 2'h0} |
                      {R0SZ{denm_lead_one[ 4]}} & {a0_prod[12: 0], 1'h0} |
                      {R0SZ{denm_lead_one[ 5]}} & {a0_prod[13: 0]}       |
                      {R0SZ{denm_lead_one[ 6]}} & {a0_prod[14: 1]}       |
                      {R0SZ{denm_lead_one[ 7]}} & {a0_prod[15: 2]}       |
                      {R0SZ{denm_lead_one[ 8]}} & {a0_prod[16: 3]}       |
                      {R0SZ{denm_lead_one[ 9]}} & {a0_prod[17: 4]}       |
                      {R0SZ{denm_lead_one[10]}} & {a0_prod[18: 5]}       |
                      {R0SZ{denm_lead_one[11]}} & {a0_prod[19: 6]}       |
                      {R0SZ{denm_lead_one[12]}} & {a0_prod[20: 7]};
...
assign  x1_prod = lut_recip_q * ((a0_prod_adj_q ^ {R0SZ{1'b1}}) + 1'b1) + x1_rnd_bit;

generate
if (PRCIS_EXT == "LVL_0") begin: gen_prod_lvl0

/*
assign  x1_prod_adj = {OPRCIS{denm_lead_one_q[ 1]}} & {       x1_prod[21 -: 14]}  |
                      {OPRCIS{denm_lead_one_q[ 2]}} & { 1'h0, x1_prod[21 -: 13]}  |
                      {OPRCIS{denm_lead_one_q[ 3]}} & { 2'h0, x1_prod[21 -: 12]}  |
                      {OPRCIS{denm_lead_one_q[ 4]}} & { 3'h0, x1_prod[21 -: 11]}  |
                      {OPRCIS{denm_lead_one_q[ 5]}} & { 4'h0, x1_prod[21 -: 10]}  |
                      {OPRCIS{denm_lead_one_q[ 6]}} & { 5'h0, x1_prod[21 -:  9]}  |
                      {OPRCIS{denm_lead_one_q[ 7]}} & { 6'h0, x1_prod[21 -:  8]}  |
                      {OPRCIS{denm_lead_one_q[ 8]}} & { 7'h0, x1_prod[21 -:  7]}  |
                      {OPRCIS{denm_lead_one_q[ 9]}} & { 8'h0, x1_prod[21 -:  6]}  |
                      {OPRCIS{denm_lead_one_q[10]}} & { 9'h0, x1_prod[21 -:  5]}  |
                      {OPRCIS{denm_lead_one_q[11]}} & {10'h0, x1_prod[21 -:  4]}  |
                      {OPRCIS{denm_lead_one_q[12]}} & {11'h0, x1_prod[21 -:  3]};


assign  x1_rnd_bit  = {X1TC_BIT{denm_lead_one_q[ 1]}} & {11'h0, 1'b1,  7'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 2]}} & {10'h0, 1'b1,  8'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 3]}} & { 9'h0, 1'b1,  9'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 4]}} & { 8'h0, 1'b1, 10'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 5]}} & { 7'h0, 1'b1, 11'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 6]}} & { 6'h0, 1'b1, 12'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 7]}} & { 5'h0, 1'b1, 13'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 8]}} & { 4'h0, 1'b1, 14'h0}  |
                      {X1TC_BIT{denm_lead_one_q[ 9]}} & { 3'h0, 1'b1, 15'h0}  |
                      {X1TC_BIT{denm_lead_one_q[10]}} & { 2'h0, 1'b1, 16'h0}  |
                      {X1TC_BIT{denm_lead_one_q[11]}} & { 1'h0, 1'b1, 17'h0}  |
                      {X1TC_BIT{denm_lead_one_q[12]}} & {       1'b1, 18'h0};
...
always @* begin
   // i = 1
   x1_rnd_bit = {X1TC_BIT{denm_lead_one_q[ 1]}} & {{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}};

   for (i = 2; i <= IPRCIS-1; i = i+1) begin
      x1_rnd_bit = x1_rnd_bit | ({X1TC_BIT{denm_lead_one_q[i]}} & ({{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}} << i-1));
   end
end
...
always @* begin
   // i = 1, 2
   x1_rnd_bit = ({X1TC_BIT{denm_lead_one_q[ 1]}} &  {{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}}) |
                ({X1TC_BIT{denm_lead_one_q[ 2]}} & ({{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}} << 1));

   for (i = 3; i <= IPRCIS-1; i = i+1) begin
      x1_rnd_bit = x1_rnd_bit | ({X1TC_BIT{denm_lead_one_q[i]}} & ({{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}} << i-3));
   end
end
...
always @* begin
   // i = IPRCIS-1
   x1_rnd_bit = {X1TC_BIT{denm_lead_one_q[IPRCIS-1]}} & ({{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}} << 1);

   for (i = 1; i <= IPRCIS-2; i = i+1) begin
      x1_rnd_bit = x1_rnd_bit | ({X1TC_BIT{denm_lead_one_q[i]}} & ({{X1TC_BIT-(X1SZ-OPRCIS){1'b0}}, 1'b1, {X1SZ-OPRCIS-1{1'b0}}} << (i+1)%2));
   end
end
...
assign  o_prcis_idx_nxt = PRCIS_EXT == "LVL_2" ? {|denm_lead_one_q[12:11]                            ,
                                                  |denm_lead_one_q[10: 9] & ~(|denm_lead_one_q[12:11]),
                                                  |denm_lead_one_q[ 8: 7] & ~(|denm_lead_one_q[12: 9]),
                                                  |denm_lead_one_q[ 6: 5] & ~(|denm_lead_one_q[12: 7]),
                                                  |denm_lead_one_q[ 4: 3] & ~(|denm_lead_one_q[12: 5]),
                                                                            ~(|denm_lead_one_q[12: 3])} :
                          PRCIS_EXT == "LVL_1" ? {|denm_lead_one_q[12:3],
                                                                           ~(|denm_lead_one_q[12:3])} : 1'b1;
